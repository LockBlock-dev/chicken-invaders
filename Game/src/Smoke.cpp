...
Smoke::Smoke(
  ...
  for (size_t i = 0; i < this->particleCount; ++i) {
    ...
    int angleOffset = random_range(0, angleSpread) - angleSpread / 2;
    int angle =
        angleOffset + baseAngle + (angleOffset + baseAngle < 0 ? 256 : 0);
    ...
    if (angleSpread != 256)
      speed =
          ((1.0 -
            std::fabs((long double)angleOffset / (long double)(angleSpread / 2))
           ) *
           (long double)speed);
...
void Smoke::update() {
  ...
  for (size_t i = 0; i < this->particleCount; ++i) {
    ...
    if (particle.speed > 0) {
      ...
      int surfaceNumber = 20 - 20 * particle.speed / 1280;
      ...
      sprite.setSurface(global::game->surface_chain_smoke->getSurf(surfaceNumber
      ));
