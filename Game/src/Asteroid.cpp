...
void Asteroid::update() {
  ...
  if (global::game->gameController->getGraphicsQuality()) {
    ...
    if (isSurfaceVisible) {
      ...
      if (shouldRender) {
        unsigned int speedRange = this->speed * 64;

        speedRange = std::clamp(speedRange, 0u, 1279u);
        ...
        global::game->gameController->asteroid_explosion_smoke_list->add(
            this->type == AsteroidType::Fire
                ? dynamic_cast<UveBase*>(new Explosion{
                      this->uveDX, this->sprite_x, this->sprite_y, 10,
                      speedRange, adjustedDirectionAngle, 128, true
                  })
