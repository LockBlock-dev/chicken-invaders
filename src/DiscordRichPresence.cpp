...
DiscordRichPresence::DiscordRichPresence(UveDX::UveDX *uveDX)
  ...
  auto result = discord::Core::Create(
      DISCORD_CLIENT_ID, DiscordCreateFlags_NoRequireDiscord, &this->core);

  if (result != discord::Result::Ok && result != discord::Result::InternalError)
    this->uveDX->onError("DiscordRichPresence::DiscordRichPresence",
                         "Failed to initialize Discord Game SDK",
  ...
  if (result == discord::Result::Ok) {
    this->core->ActivityManager().UpdateActivity(
        this->activity, [this](discord::Result result) {
          if (result != discord::Result::Ok)
            this->uveDX->onError("DiscordRichPresence::DiscordRichPresence",
...
void DiscordRichPresence::update() {
  ...
  if (result != discord::Result::Ok)
    this->uveDX->onError("DiscordRichPresence::update",
                         "Failed to run callbacks", static_cast<int>(result));
}

bool DiscordRichPresence::isInitialized() const { return this->initialized; }
