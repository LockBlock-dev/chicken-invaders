...
void Animator::loadFromFile(const std::string& filename) {
  ...
  this->fileStream.read(
      reinterpret_cast<char*>(&this->width), sizeof(this->width)
  );
  this->fileStream.read(
      reinterpret_cast<char*>(&this->height), sizeof(this->height)
  );
  this->fileStream.read(
      reinterpret_cast<char*>(&this->framerate), sizeof(this->framerate)
  );
...
void Animator::process() {
  ...
  while (true) {
    this->fileStream.read(
        reinterpret_cast<char*>(&interactionType), sizeof(interactionType)
    );
    ...
    switch (interactionType) {
      ...
      case 1: {
        ...
        this->fileStream.read(
            reinterpret_cast<char*>(&frameSize), sizeof(frameSize)
        );
        ...
        { uint16_t pixelCount = 0;
            ...
            auto bytesToCopy = *reinterpret_cast<uint16_t
        *>(frameDataBufferCopy);
      ...
      case 2: {
        ...
        this->fileStream.read(
            reinterpret_cast<char*>(&sleepTime), sizeof(sleepTime)
        );
